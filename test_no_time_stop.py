"""
test_no_time_stop.py
====================
Backtest testing target ratios (1:1.0, 1:1.2, 1:1.5, 1:2.0) WITHOUT the 5-candle time stop
(trades run until they hit either TP or SL price level).
Runs on 1-Month MT5 data across Top 8 pairs.
"""
...
MIN_SPIKE_CANDLES = 2
BODY_RATIO_MIN    = 0.50
MAX_HOLD_CANDLES  = 500  # No time stop - run until TP or SL hit
DAYS_BACK         = 30
...
def run_rr_test_no_time_stop(setups, ltf_df, mode, rr_target):
    used_idx = set()
    tp_hits = 0
    sl_hits = 0
    net_pnl = 0.0
    ...
    for s in setups:
        ...
        if result == 'WIN':
            ...
            # Unfinished at end of dataset
            pass

        used_idx.add(i)

    total_closed = tp_hits + sl_hits
    win_rate = (tp_hits / total_closed * 100) if total_closed > 0 else 0
...
def main():
    ...
    ratios = [1.0, 1.2, 1.3, 1.4, 1.5, 2.0]
    comparison = {rr: {'total_setups': 0, 'tp_hits': 0, 'sl_hits': 0, 'win_rate': 0, 'net_pnl': 0.0} for rr in ratios}
    ...
    for display_name, meta in PAIRS.items():
        ...
        for rr in ratios:
            res = run_rr_test_no_time_stop(setups, ltf_df, mode, rr)
            comparison[rr]['total_setups'] += res['total_setups']
            comparison[rr]['tp_hits'] += res['tp_hits']
            comparison[rr]['sl_hits'] += res['sl_hits']
            comparison[rr]['net_pnl'] += res['net_pnl']

    for rr in ratios:
        tot = comparison[rr]['tp_hits'] + comparison[rr]['sl_hits']
        comparison[rr]['win_rate'] = round((comparison[rr]['tp_hits'] / tot * 100), 2) if tot > 0 else 0

    mt5.shutdown()

    out_path = os.path.join(os.path.dirname(__file__), "cache", "no_time_stop_results.json")
    with open(out_path, "w") as f:
        json.dump(comparison, f, indent=2)

    print("NO_TIME_STOP_TEST_DONE")
