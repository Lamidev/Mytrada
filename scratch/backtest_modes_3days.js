...
function runModeStrategy(symbolsData, modeName, opts, filterStartTime) {
  const {
    rewardRatio = 1.4,
    atrMultiplier = 1.5,
    minBodyRatio = 0.50,
  ...
  for (const [sym, data] of Object.entries(symbolsData)) {
    ...
    for (let i = 20; i < ltf.length; i++) {
      ...
      if (useHtfDistance) {
        ...
        if (emaDistPct < 0.0008) continue; // Skip if directly hovering on 50 EMA line
      }
      ...
      if (pairMode === 'BOOM') {
        ...
        if (useRSIFilter && rsiValues) {
          const rsiPeak = Math.max(rsiValues[i - 1] || 50, rsiValues[i - 2] || 50);
          if (rsiPeak < 65) continue;
        }
        ...
        if (useRSIFilter && rsiValues) {
          const rsiTrough = Math.min(rsiValues[i - 1] || 50, rsiValues[i - 2] || 50);
          if (rsiTrough > 35) continue;
        }
...
function main() {
  ...
  const modes = [
    {
      id: "CURRENT",
      title: "1. Current Baseline Mode (1:1.3 R:R | 2 Spikes | No Extra Filters)",
      opts: { rewardRatio: 1.3, atrMultiplier: 1.5, minBodyRatio: 0.50, minSpikes: 2 }
    },
    {
      id: "BALANCED",
      title: "2. High-Action Balanced Mode (1:1.4 R:R | 1H Trend Clearance | 2 Spikes)",
      opts: { rewardRatio: 1.4, atrMultiplier: 1.5, minBodyRatio: 0.50, minSpikes: 2, useHtfDistance: true }
    },
    {
      id: "3_SPIKES",
      title: "3. Deep Exhaustion Mode (1:1.4 R:R | 3+ Consecutive Spikes)",
      opts: { rewardRatio: 1.4, atrMultiplier: 1.5, minBodyRatio: 0.50, minSpikes: 3 }
    },
    {
      id: "ULTRA_SNIPER",
      title: "4. Ultra-Sniper Mode (1:1.4 R:R | RSI Overbought >65 / Oversold <35)",
      opts: { rewardRatio: 1.4, atrMultiplier: 1.5, minBodyRatio: 0.50, minSpikes: 2, useRSIFilter: true }
    }
  ];

  console.log("==========================================================================================");
  console.log("📊 3-DAY HEAD-TO-HEAD BACKTEST: $100 ACCOUNT (AUG 13 - AUG 15)");
  console.log("==========================================================================================\n");

  const comparisonTable = [];

  modes.forEach(mode => {
    const trades = runModeStrategy(symbolsData, mode.id, mode.opts, augustStartTime);
    ...
    comparisonTable.push({
      "Strategy Mode": mode.title,
      "Trades": total,
      "Wins": wins,
      "Losses": losses,
      "Win Rate": `${winRate}%`,
      "Fixed $3 Net Profit": `+$${simFixed.netProfit.toFixed(2)} (+${simFixed.returnPct.toFixed(1)}%)`,
      "Fixed Final Balance": `$${simFixed.finalBalance.toFixed(2)}`,
      "Max DD (Fixed)": `-$${simFixed.maxDrawdownUSD.toFixed(2)} (${simFixed.maxDrawdownPct.toFixed(1)}%)`,
      "Compound Final Balance": `$${simCompound.finalBalance.toFixed(2)} (+${simCompound.returnPct.toFixed(1)}%)`
    });

    console.log(`------------------------------------------------------------------------------------------`);
    console.log(`📌 ${mode.title.toUpperCase()}`);
    console.log(`   • Total Closed Trades: ${total} (${wins} Wins / ${losses} Losses)`);
    console.log(`   • Win Rate:            ${winRate}%`);
    console.log(`   • Fixed $3 Risk:       +$${simFixed.netProfit.toFixed(2)} Net Profit ➔ Final Balance: $${simFixed.finalBalance.toFixed(2)}`);
    console.log(`   • Max Drawdown:        -$${simFixed.maxDrawdownUSD.toFixed(2)} (${simFixed.maxDrawdownPct.toFixed(1)}%)`);
    console.log(`   • Dynamic Compounding: +$${simCompound.netProfit.toFixed(2)} Net Profit ➔ Final Balance: $${simCompound.finalBalance.toFixed(2)}`);
